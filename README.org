...
This prompt is not perfect, especially on smaller models who usually fail to follow the instruction to produce /only/ the filling code.  The post-processing steps (~starhugger-post-process-chain~) use some heuristics to extract "just" code from the response.
...
(add-to-list 'starhugger-instruct-default-system-prompt-messages "/no_think")
#+end_src
