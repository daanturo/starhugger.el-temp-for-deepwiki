...
- Add OpenAI-compatible API support make it the default, the default base URL is still Ollama's.
  
- Removals:
  - Hugging Face's old text inference backend (https://github.com/daanturo/starhugger.el/issues/3).
